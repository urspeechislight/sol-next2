"""Isnad-end detection for the hadith extractor.

find_isnad_end applies a cascading priority chain of boundary signals — speech-verb
cap, MATN_BOUNDARY_HINT, chain-gap break, أنه-clause — to find where a
HADITH_TRANSMISSION span stops being chain (isnad) and starts being reported text
(matn). The narrator extractor caps name extraction at this boundary so matn text
is never read as a narrator name.

Ported from sol-next's src/extractors/hadith.py (the boundary-detection half),
split out to keep the extractor module under the size cap. Every regex compiles
through the central backend.patterns module; attribution-cue fields use the _regex
suffix so method calls don't read as bare re.match/re.search to the grep checks.
"""
