...
_HEADING_LEADS: Final[frozenset[str]] = _norm_set(
    (
        "طبقته", "طبقاته", "شخصيته", "ترجمته", "رواياته", "روايته", "وثاقته", "أحواله",
        "احواله", "نسبه", "كنيته", "مشايخه", "تلاميذه", "تلامذته", "شيوخه", "وفاته",
        "مولده", "اسمه", "لقبه", "عصره", "حياته", "سيرته", "مكانته", "منزلته", "آثاره",
    )
)
...
def clean_name(name: str) -> str:
    ...
    for i in range(1, len(surface)):
        if norm[i] in _STOP_TOKENS or norm[i] in _CONNECTIVE_VERBS:
            cut = i
            break
    surface = surface[:cut]
