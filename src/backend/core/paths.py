...
``data/`` + ``config/`` directories are located.

Everything that needs an on-disk path under the repo derives it from here:
the settings ``.env`` lookup, the static-data loader, the build scripts, and
the build-input manifests under ``config/``. The root is computed once so it
cannot drift between callers. ``REPO_ROOT`` walks four parents up from this
file: ``core/paths.py`` to ``core`` to ``backend`` to ``src`` to the repo
root.
"""
...
def config_path(file_name: str) -> Path:
    """Return the absolute path of ``file_name`` under the repo-root ``config/``."""
