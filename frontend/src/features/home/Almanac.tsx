...
import type { Almanac as AlmanacData } from '../../lib/types';
...
/** The almanac band: the chronicle entry for today beside the Hijri calendar,
    where the month's observances are listed and today is marked
    typographically (accent numeral + tag), never with a highlight bubble.
    Fetches the served almanac once; the client computes "today" and selects. */
export function Almanac({ today }: AlmanacProps) {
  const almanac = useAsync<AlmanacData>(() => getAlmanac(), []);
  return (
    <DataView result={almanac} renderLoading={() => null} errorText="Could not load the almanac">
      {(data) => (
        <section className="almanac" aria-label="Almanac">
          <HistoryCard almanac={data} today={today} />
          <HijriCard almanac={data} today={today} />
        </section>
      )}
    </DataView>
  );
...
function HijriCard({ almanac, today }: { almanac: AlmanacData; today: HijriToday }) {
...
function HistoryCard({ almanac, today }: { almanac: AlmanacData; today: HijriToday }) {
