...
export function Pager({ page, totalPages, onPage, className }: PagerProps) {
  ...
  return (
    <nav className={cx("ds-pager", className)} aria-label="Pagination">
      <Button variant="ghost" size="sm" iconBefore="chevron-left" ariaLabel="Previous page"
        disabled={page <= 1} onClick={() => onPage(page - 1)} />
      {window[0] > 1 ? <span className="ds-pager__gap">…</span> : null}
      {window.map((p) => (
        <button key={p} type="button" aria-current={p === page}
      ...
      <Button variant="ghost" size="sm" iconBefore="chevron-right" ariaLabel="Next page"
