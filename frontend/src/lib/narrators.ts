...
export function annotateText(text: string, index: NarratorIndex | null): TextSegment[] {
  ...
  while (i < parts.length) {
    const part = parts[i];
    if (!isWord(part)) { buffer += part; i += 1; continue; }

    const match = matchAt(parts, i, index);
    if (match) {
      if (buffer) { segs.push({ type: "text", value: buffer }); buffer = ""; }
      segs.push({ type: "narrator", value: parts.slice(i, match.endIndex + 1).join(""), record: match.record });
      i = match.endIndex + 1;
    } else {
      buffer += part;
      i += 1;
...
interface Match { endIndex: number; record: NarratorRecord }
...
function matchAt(parts: string[], start: number, index: NarratorIndex): Match | null {
  ...
  for (const { toks, record } of candidates) {
    ...
    while (ti < toks.length && pi < parts.length) {
      const part = parts[pi];
      if (!part || /^\s+$/.test(part)) { pi += 1; continue; }
      const norm = normalizeArabic(part);
      if (norm === toks[ti]) { lastWordIndex = pi; pi += 1; ti += 1; continue; }
      // allow a connective (bn/ibn/an...) in the text that the name omits
      if (STOP.has(norm) && ti > 0) { pi += 1; continue; }
      break;
    }
...
export function rijalToRecord(e: RijalEntry): NarratorRecord {
  return {
    id: e.id, full_name: e.full_name, kunya: e.kunya, nisba: e.nisba, tradition: e.tradition,
    birth_year: e.birth_year, death_year: e.death_year,
    teacher_count: e.teacher_count, student_count: e.student_count,
    reliability_term: e.reliability_term, reliability_grade: e.reliability_grade,
    evaluator: e.evaluator, source_label: e.source_label, origin: "rijal",
  };
}

export function canonicalToRecord(e: CanonicalEntry): NarratorRecord {
  return {
    id: e.canonical_id, full_name: e.full_name, kunya: e.kunya, nisba: e.nisba, tradition: e.tradition,
    birth_year: String(e.birth_year ?? ""), death_year: String(e.death_year ?? ""),
    teacher_count: e.teacher_count, student_count: e.student_count,
    merge_confidence: e.merge_confidence, origin: "canonical",
  };
}
...
export function mergeNarrators(rijal: NarratorRecord[], canonical: NarratorRecord[]): NarratorRecord[] {
