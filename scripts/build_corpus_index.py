...
``backend.build.corpus``; this script only projects rows. Run on buildhost::
...
def build(out: Path) -> tuple[int, int]:
    ...
    con = corpus_build.create_index(out)
    ...
    try:
        for book in books:
            ...
            if not rows:
                ...
                n_empty += 1
                continue
            corpus_build.insert_pages(con, rows)
            n_books += 1
            n_pages += len(rows)
        ...
        con.commit()
        corpus_build.build_book_table(con, corpus_build.book_table_rows(books))
        con.commit()
    finally:
        con.close()
