...
| Raw SQL strings                                         | **enforced (block)** | `centralization` (CENTRAL-005): only in `src/backend/repositories/` or migrations                                                                    |
...
| Secret-store path refs (`REDACTED://`, `vault://`, ARN) | **enforced (block)** | `centralization` (CENTRAL-018): only in settings module                                                                                              |
